...
Claude CoworkやOpenClawのような**完全自律型エージェント**が注目を集めています。しかしエンタープライズの現場では、コンプライアンス要件、承認フロー、監査証跡といった制約から、**AIにすべてを任せきる完全自律型がそのまま適用できるケースは限られます**。むしろ「AIが処理を進めつつ、要所では人間が判断・承認する」**半自律型のアプローチ**——すなわち **Agentic Workflow** ——が、現実的な落としどころとして、特にエンタープライズなど制約の大きい領域では、完全自律型から半自律型へのゆり戻しが起きるのではないかと考えています。
...
[^dev_voice]: [Grokでの開発者の声のまとめ](https://grok.com/share/c2hhcmQtMw_032907d4-882e-45fd-9ad4-ab7698e64c1b)。急峻な学習曲線、抽象化の層が深すぎて複雑といった意見が多い。関連記事: [LangGraphが使いづらいので代わりのフレームワークを探してみた](https://note.com/philoludum/n/n771f7b7f8e51)、[LangGraph: A Rant (Reddit)](https://www.reddit.com/r/LangChain/comments/1jc2am4/langgraph_a_rant/?tl=ja)、[LangGraphの学習コストが高すぎたので改めてワークフローフレームワークについて考えてみた](https://zenn.dev/kurarrr/articles/c001548c981aca)
...
:::message
LangGraphのAPIがこのような冗長な設計になっている背景には、DAGでは表現できない**循環（ループ）を含むグラフ**をサポートする必要があるという事情が考えられます。Graflowではループをグラフ構造ではなく実行時の動的制御で解決しており（→ 壁2参照）、グラフ定義自体はDAGの簡潔さを保っています。
:::

### 壁2: 条件分岐とループを事前定義しなければならない

`add_conditional_edges` ですべての分岐パスをコンパイル時に固定する必要があり、実行時にデータ量やスコアに応じて柔軟に分岐するのが困難です。

```python
# LangGraph: ルーティング関数 + 分岐先の事前定義
...
graph.add_conditional_edges(
    "evaluate",
    route_after_eval,
    {"publish": "publish", "retry": "retry", "fallback": "fallback"},  # すべてのパスを事前列挙
)
# ループも add_edge で明示的にサイクルを張る
...
def evaluate(context: TaskExecutionContext, row):
    if row["score"] >= 0.8:
        context.next_task(publish_taskfn)       # 条件分岐
    elif row["retry_count"] < 3:
        context.next_iteration()                # ループ（再実行）
    else:
        context.next_task(fallback_taskfn)
```

Graflowでは **`next_task()` / `next_iteration()` で実行時に動的に制御**でき、分岐先の事前列挙やグラフ上のサイクル定義が不要です。
...
Graflowは **Redisをタスクキューとした分散ワーカーをOSSとして標準装備**しており、Apache Airflowの Celery Executor と同様に、タスクを複数のワーカーマシンに分散して水平スケールできます。ローカル実行から分散実行への切り替えは1行で可能です。
...
Graflowはこの依存チェーンを断ち切り、**LiteLLM統合でプロバイダー非依存**を実現しています。SuperAgentは **Google ADK や PydanticAI 等の専門フレームワークをそのまま利用**できます（[Strands Agents も対応予定](https://github.com/GraflowAI/graflow/issues/44)）。
...
:::message
**ポイント**: LangGraphではすべてのノードが共有State（TypedDict）を介してデータをやり取りし、Stateスキーマの事前定義とReducerの設定が必要です。Graflowでは、データ共有が必要な場合だけChannelの `set`/`get` で明示的にやり取りします。Channelの詳細は[第4章](#4-タスク間のデータ共有--reducerは不要)で解説します。
:::
...
特に重要なのが**並列実行時**です。[pharmax さんの解説記事](https://zenn.dev/pharmax/articles/78f2e6a51a459e#%E4%B8%A6%E5%88%97%E5%AE%9F%E8%A1%8C%E6%99%82%E3%81%AEstate%E3%81%AB%E3%81%AFreducer%E3%81%8C%E5%BF%85%E8%A6%81)で詳しく解説されている通り、LangGraphでは並列分岐するノードがStateの同じフィールドを更新する場合、Reducerの指定が**必須**です。Reducerがないと更新順序が不定となり、`"Already found path for node. For multiple edges, use StateGraph with an annotated state key."` というエラーが発生します。つまり、**並列実行を安全に行うには、事前にStateスキーマでReducer関数を宣言しておく必要がある**という設計です。

Graflowはこの問題に対して異なるアプローチを取ります。Reducerという事前宣言の仕組みは不要で、代わりに**タスク内部で並行安全なプリミティブを明示的に使う**設計です。
...
def safe_counter(context: TaskExecutionContext):
    ...
    # 1. atomic_add: アトミックなカウンタ更新（ロスト・アップデートなし）
    channel.atomic_add("processed_count", 1)

    # 2. append/prepend: リストへのスレッドセーフな追加
    #    LangGraph の add_messages Reducer に相当する操作を、宣言なしで実行
    channel.append("logs", "Processing completed")
    channel.prepend("priority_queue", "urgent_item")

    # 3. lock: 複合的な読み取り→判定→書き込みをアトミックに実行
    with channel.lock("counter"):
        val = channel.get("counter")
        if val >= 100:
            channel.set("counter", 0)          # リセット
            channel.atomic_add("overflow", 1)  # オーバーフロー回数を記録
        else:
...
さらに、**自動キーワード引数解決**[^auto_kwargs]を使えば `inject_context` すら不要です:
...
[^auto_kwargs]: 自動キーワード引数解決は、タスク関数のシグネチャを `inspect.signature()` で解析し、引数名に一致するキーがChannelに存在すれば自動的に値を注入する仕組みです。デフォルト値を持つ引数はChannelにキーがなければデフォルト値が使われ、デフォルト値のない引数はChannelにキーがなければエラーになります。これにより、上流タスクが `channel.set("user_name", ...)` で書いた値を、下流タスクは `def greet(user_name: str)` と宣言するだけで受け取れます。
...
LangGraphでも `RetryPolicy` によるノード単位のリトライは可能です（[8章参照](#タスク単位のリトライretrypolicy)）。また、イテレーション（同一タスクの意図的な繰り返し）についても、`add_conditional_edges` でグラフにサイクルを事前定義し、State内にカウンタや収束条件を持たせることで実現できます。ただし、ループの構造（どのノードからどのノードに戻るか）はグラフ構築時に宣言する必要があり、繰り返し回数の上限も State のロジックか `recursion_limit` で自前管理する設計です。

Graflowでは `next_iteration()` を呼ぶだけでタスク自身を再実行でき、`@task(max_cycles=N)` で上限を宣言的に制御できます。サイクルの事前定義やState内のカウンタ管理が不要で、タスクの中で「もう一度実行するかどうか」を実行時に決定できます。

| 機能 | LangGraph | Graflow |
|---|---|---|
| **リトライ（失敗時の自動再試行）** | `RetryPolicy`（`add_node` で指定） | `RetryPolicy`（`@task` で指定）+ `default_max_retries` |
| **イテレーション（意図的な繰り返し）** | `add_conditional_edges` でサイクルを事前定義 + State内カウンタ + `recursion_limit` | `@task(max_cycles=N)` + `next_iteration()` |
...
LangGraphの `interrupt()` は、内部的には**例外（`GraphInterrupt`）を送出してグラフの実行を中断**する仕組みです[^interrupt_exception]。Pythonの `raise` / `try-except` に似た制御フローであり、ノード関数の途中で呼ばれると、それ以降の処理は実行されずにグラフ全体が停止します。

[^interrupt_exception]: `GraphInterrupt` は例外オブジェクトなので、実際のところ `try` / `except` でキャッチしてハンドリングできます。例外処理に慣れたPython開発者にとっては、この仕組み自体は直感的で扱いやすいとも言えます。一方で、チェックポイント＋ `Command(resume=...)` による再開フローと組み合わせると、例外がどこで発生し・どこでキャッチされ・どの状態で再開されるかの把握が難しくなる場面があります。
...
Kubernetes環境では HPA（Horizontal Pod Autoscaler）で[Redisキューの残タスク数に基づきワーカーPod数を自動調整](https://github.com/GraflowAI/graflow/blob/main/docs/scaling/kubernetes_autoscaling.md)できます。[Apache Airflow の Celery Executor](https://qiita.com/mizukyf/items/29832c83011030f778c0) と同様のアーキテクチャです。
...
| `docker` | コンテナ実行 | GPU処理、依存ライブラリの隔離、サンドボックス実行 |
| カスタム | 自由に実装可能 | Cloud Run、リモート実行など |

**Dockerハンドラーの利点:**

- **依存ライブラリの隔離**: タスクが必要とするライブラリをコンテナ内に閉じ込めることで、ワークフローランタイム側のライブラリとのバージョン競合を回避できます。例えば、あるタスクが `pandas 1.x` を必要とし、別のタスクが `pandas 2.x` を必要とする場合でも、それぞれ異なるDockerイメージで実行すれば問題になりません。
- **サンドボックス実行**: LLMが生成したコードをホスト環境から隔離されたコンテナ内で安全に実行できます。生成コードがファイルシステムやネットワークに意図しないアクセスを行うリスクを、コンテナのサンドボックスで軽減できます。
...
LangGraphでは、`MessagesState`（メッセージ履歴の自動追加Reducer）や`ChatOpenAI`など、LangChainエコシステムとの統合が前提です。エージェント構築には `create_react_agent` や `ToolNode` を使い、ツール呼び出しの判定・実行までワークフローグラフのノードとエッジで表現する設計になっています。これにより、本来エージェント内部で完結すべきツール選択のロジックまでグラフ定義に漏れ出し、ワークフローの見通しが悪くなりがちです。

### Graflow: フレームワーク非依存

Graflowは**ワークフローのオーケストレーション**と**エージェントのツール呼び出し**を明確に分離しています。ツール選択・実行はエージェント（SuperAgent）側の責務とし、ワークフロー側はタスクの流れの制御に専念します。この責務分離により、2つのLLM統合方式を提供しつつも**特定のLLMフレームワークに依存しません**。
...
Graflowの設計思想は「SuperAgent（ReActループ等）は専門フレームワーク（Google ADK、PydanticAI等）に委譲し、ワークフローオーケストレーションに集中する」というものです。LangGraphがSuperAgentもワークフローも自前で実装するフルスタック型であるのに対し、Graflowは責務分離型のアプローチを取っています。このコンセプトの詳細は「[Graflowスタイル: SuperエージェントはFatノード](https://zenn.dev/myui/articles/64f31faaf1488c#%E2%9C%85-graflow%E3%82%B9%E3%82%BF%E3%82%A4%E3%83%AB%3A-super%E3%82%A8%E3%83%BC%E3%82%B8%E3%82%A7%E3%83%B3%E3%83%88%E3%81%AFfat%E3%83%8E%E3%83%BC%E3%83%89)」を参照してください。
...
```mermaid
graph TB
    subgraph Graflow["Graflow（ワークフローオーケストレーション）"]
        direction TB
        G["タスク依存関係・並列実行・HITL・チェックポイント"]
        subgraph Agent["SuperAgent（差し替え可能）"]
            A["Google ADK / PydanticAI / OpenAI Agents SDK / ..."]
            B["ReActループ・ツール選択・実行・推論"]
        end
    end
```

- **SuperAgent**（Google ADK、PydanticAI、OpenAI Agents SDK 等）がReActループ（LLM推論 → ツール選択 → ツール実行 → 応答生成）を**内部で完結**
- **Graflow** はタスク間の依存関係、HITL承認、チェックポイント、分散実行を担当
- SuperAgentは `inject_llm_agent` で差し替え可能。**どのフレームワークを選んでもワークフロー側のコードは変わりません**
...
ここまでの比較を通じて見えてきたのは、LangGraphとGraflowの違いが単なるAPIの差ではないということです。**コンパイル時に構造を固定するか、実行時に構造を決定するか**——この設計思想の違いが、分岐・並列・中断再開・スケーリングといったすべての機能差として表れています。Graflowは、この設計思想のもとで開発者体験（Developer Experience）を徹底的に追求したフレームワークです。
...
**LangGraphが向いているケース:**
- フローが事前に確定しており、変更頻度が低い
- グラフの可視化・静的検証を重視したい
- LangChainエコシステム（LangSmith等）を既に活用している

### ワークフローか自律エージェントか——自律性スライダーという視点

そもそも、AIワークフローと自律エージェントの選択は二者択一ではありません。Andrej Karpathyが[「自律性スライダー」](https://www.decodingai.com/p/ai-workflows-vs-agents-the-autonomy)と呼ぶように、自律性は信頼とリスクに応じて調整される連続的なスライダーです。決定論的なワークフローから完全自律のエージェントまで、ユースケースごとに最適なポイントは異なります。

だからこそ重要なのは、このスライダーのどの位置でも対応できるオーケストレーション基盤を持つことです。Graflowでは、各タスクに委譲するSuperAgent（ADK、PydanticAI等）の自由度とカバー範囲を上げればスライダーは自律寄りに、確定的なパイプラインで組めばワークフロー寄りに——同じフレームワーク内でスライダーの位置を自在に調整できます。

### スタンドアロンSDKに対する見解

OpenAI Agents SDKやGoogle ADKなどの**スタンドアロンのエージェントSDK単体**でワークフローを組む選択肢もあります。シンプルなPoCやプロトタイプではこのアプローチは有効です。

しかし、プロダクション環境で求められる要件——**循環（ループ・リトライ）、チェックポイントによる中断・再開、Human-in-the-Loop、エラーハンドリングとリトライポリシー、複数LLMプロバイダーの使い分け、プロンプトのバージョン管理**——を満たそうとすると、結局オーケストレーションフレームワーク相当の機能を自前で作り込むことになります。その開発・保守コストは決して小さくありません。

Graflowはこれらの機能をフレームワークとして標準提供しつつ、エージェントSDK側の選択は自由に委ねる設計です。「オーケストレーションはGraflow、エージェントのツール呼び出しや推論はお好みのSDK」という責務分離により、車輪の再発明を避けながら柔軟な構成が可能になります。
...
| 4 | **プロダクション対応** | checkpoint/resume、HITL、Dockerハンドラー |
| 5 | **並列分散処理によるスケーリング** | Airflow風の水平スケール。ローカル実行→分散実行の切り替えが1行で可能 |

---

## Graflowを始めてみよう

GraflowはApache 2.0ライセンスのOSSです。[GitHub](https://github.com/GraflowAI/graflow)でのIssue報告やPull Requestも歓迎しています。
...
最近リリースされたばかりで話題の **[Gemma 4](https://ollama.com/library/gemma4:e4b)** などのローカルLLMを利用した **Google ADK** や **LLM completion** の実行例も含まれています。ローカルLLMを活用することで、秘匿性の高いデータを外部に送信せずにワークフロー内で安全に処理できます。ぜひ実際に触って、Graflowの開発体験を確かめてみてください。
:::

:::message
**💡 Claude Code SkillsによるAIネイティブな開発体験**

Graflowは [Claude Code](https://docs.anthropic.com/en/docs/claude-code) の **Custom Skills**（スラッシュコマンド）に対応しています。自然言語で指示するだけで、Graflowのワークフローやタスクのスケルトンコードを生成できます。

例えば、Claude Codeのプロンプトで以下のように指示するだけで、ワークフローの雛形が生成されます:

```
/graflow-workflow「ユーザーの質問をWebで検索し、結果を要約して返すワークフローを作って」
```

Claude CodeがGraflowのAPI・設計パターンを理解した上でコードを生成するため、ドキュメントを逐一参照しなくても正しいワークフロー構造が得られます。詳しくは [graflow-examples](https://github.com/GraflowAI/graflow-examples) リポジトリを参照してください。
:::
